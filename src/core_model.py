...
# Factory
# ------------------------------
def get_llm_from_env() -> BaseLLMClient:
...
class StubLLM(BaseLLMClient):
    """
    연구/테스트용 더미 LLM.
    실제 외부 호출 없이, 항상 안전한/고정된 응답을 반환.
    """
    def generate(self, messages, *, temperature: float = 0.2,
                 max_tokens: int = 512, stop=None) -> str:
        # 필요한 경우 messages 기반으로 살짝 더 똑똑하게 흉내낼 수도 있음
